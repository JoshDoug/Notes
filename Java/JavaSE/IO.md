...
Invoking `print` or `println` outputs a single value after converting the value using the appropiate toString method.

When a variable is used alone in a `print()` method, the usage is of an overloaded print method (think method overloading with a method for each type).
When a variable is concatenated within the arguments of a method the conversion code is auto generated by the Java compiler, which also utilises toString. Both ways work fine, but there is a technical difference to what goes on under the hood that is interesting to note.
